...
func _on_Quit_to_Main_Menu_pressed():
	...
	if GameManager.game_type == "new":
		...
		get_node("Save Game/Save Menu/Save Options").position = Vector2(160, 113)
		...
		#center the buttons on the screen
		get_node("Save Game/Save Menu/Save Options").position = Vector2(160, 101)
...
func _pause_game():
	...
	position = Vector2(Game.player.position.x - size.x/2, Game.player.position.y - size.y/2) + Game.player_location.position
