...
func _ready():
	_set_texture_filter_nearest(self)
	get_node("/root/Game/Farm/Player/UI/Dashboard/TimeManager").connect("sleep", Callable(self, "_force_sleep"))

func _set_texture_filter_nearest(node: Node) -> void:
	if node is CanvasItem:
		node.texture_filter = CanvasItem.TEXTURE_FILTER_NEAREST
	for child in node.get_children():
		_set_texture_filter_nearest(child)
...
func _input(event):
	...
	if Input.is_action_pressed("E") and Town_Grid.local_to_map(Player.position) == Vector2(27, 43) and not Inventory.visible and not visible:
		...
		position = Vector2(Game.player.position.x - ((size.x/2)*scale.x), Game.player.position.y - ((size.y/2)*scale.y) + 13) + Game.player_location.position
	#close the shop
