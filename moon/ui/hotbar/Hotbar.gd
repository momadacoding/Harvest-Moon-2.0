...
func _move_indicator():
	Indicator.position = Vector2(IndicatorBasePosition.x + ((IndicatorSlot-1) * HotbarItemSeparation), IndicatorBasePosition.y)
	...
	if IndicatorSlot == 3:
		Indicator.scale = Vector2(1.01, 1.01)
		var currentPosition = Indicator.position
		Indicator.position = Vector2(currentPosition.x-1, currentPosition.y)
	else:
