...
func farm_to_house():
	...
	player.position = HouseMap.map_to_local(farm_to_house_spawn) + half_tile_size
...
func house_to_farm():
	...
	player.position = FarmMap.map_to_local(house_to_farm_spawn) + half_tile_size
...
func farm_to_town():
	...
	player.position = TownMap.map_to_local(farm_to_town_spawn) + half_tile_size
	SoundManager.stop_music("farm")
...
func town_to_farm():
	...
	Farm.add_child(player)
	player.set_owner(Farm)
	player.position = FarmMap.map_to_local(town_to_farm_spawn) + half_tile_size
...
func teleport_player_to_bed():
	...
	House.add_child(player)
	player.set_owner(House)
	player.position = HouseMap.map_to_local(sleep_spawn) + half_tile_size
	Shaders.toggle_shaders() #hides shaders
	Weather.toggle_weather() #hides weather
	SoundManager.set_music_volume("rain", -10) #lower the volume of the rain, the player is going indoors
	player_location = House
