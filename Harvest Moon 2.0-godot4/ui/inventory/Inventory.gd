...
func _input(event):
	...
	if Input.is_action_pressed("Tab") and Player.speed == 0 and not Player.animationCommit and not DraggedItem.visible and not ShopMenu.visible:
		if visible:
			...
			Indicator.position = Vector2(HotbarIndicator.position.x, 394)
			Indicator.scale = HotbarIndicator.scale
...
func _physics_process(delta):
	DraggedItem.position = get_local_mouse_position() - InventoryOffset
