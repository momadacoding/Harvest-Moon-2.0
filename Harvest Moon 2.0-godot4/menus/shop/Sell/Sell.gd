...
func _move_chosen_item_indicator(position):
	...
	Chosen_Item_Indicator.position = Vector2(300 + ((((indicator_position-1) % 4))*362), 334 + (((indicator_position-1)/4)*362))
	_update_value_per_item_and_currently_have()
	amount = 0
...
func _update_value_per_item_and_currently_have():
	...
	You_Currently_Have.get("theme_override_fonts/font").size = 100
	...
	if itemName.ends_with("Seeds"):
		...
		You_Currently_Have.get("theme_override_fonts/font").size = 80
