...
func list_saves():
	...
	while true:
		...
		if file_name == "":
			...
			var instanced_button = button.instantiate()
			instanced_button.set_text(file_name.substr(0, file_name.length()-4))
			instanced_button.pressed.connect(_on_button_pressed.bind(index))
	...
	if num_buttons == 0:
		...
		Saves.size = Vector2(202, 32)
		Saves.position = Vector2(380, 336)
		get_node("Back to Main Menu/Container/Button").grab_focus()
		return
	
	#center the saves node based on the number of buttons generated
	var buttons_length = Buttons.get_child(0).size.x
	var buttons_height = (num_buttons*20) + ((num_buttons-1) * 4)
	
	Buttons.size = Vector2(buttons_length, buttons_height)
	Saves.scroll_horizontal_enabled = false
	
	#no scroll bar is required
	if num_buttons <= 10:
		Saves.scroll_vertical_enabled = false
		Saves.size = Vector2(buttons_length, buttons_height)
	else: #num_buttons > 10
		Saves.scroll_vertical_enabled = true
		Saves.size = Vector2(buttons_length+12, 236)
	
	Saves.position = Vector2((1366/2)-((Saves.scale.x*Saves.size.x)/2), (768/2) - ((Saves.scale.y*Saves.size.y)/2))
