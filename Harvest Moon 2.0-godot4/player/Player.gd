...
func _ready():
	get_node("UI/Dashboard/TimeManager").sleep.connect(_force_sleep)
...
func _force_sleep():
	if Time.get_ticks_msec() > sleepTime + sleepDelay:
		$Sprite2D.set_offset(Vector2()) #reset animation if the player was in the middle of performing an action
		...
		sleepTime = Time.get_ticks_msec() #prevent sleep spam
...
func _physics_process(delta):
	...
	if (Input.is_action_pressed("ui_accept") and not holdingItem and not animationCommit and Zone.name == "Farm" and EnergyBar.has_energy()) or (powerHold):
		...
		#spread seeds
			...
			if not powerHold:
				...
				powerHold = false
				if Time.get_ticks_msec() - holdTime >= holdTimeDelay: #the player was holding the button for more than 1 second, perform a super move
					lastAnimation = "sickle circle"
			elif $Sprite2D.get_frame() == 7: #powerHold is true, player is still holding the button
	...
	if Input.is_action_pressed("E") and Zone.name == "House" and Zone.can_sleep(position) and !is_moving and Time.get_ticks_msec() > sleepTime + sleepDelay:
	...
	if not is_moving and direction != Vector2() and not Inventory.visible:
		...
		if stepLeft and not SoundManager.is_playing("rightFoot") and Time.get_ticks_msec() >= stepDelay + stepTime:
			SoundManager.play_effect("leftFoot")
			stepRight = true
			stepLeft = false
			stepTime = Time.get_ticks_msec()
		elif stepRight and not SoundManager.is_playing("leftFoot") and Time.get_ticks_msec() >= stepDelay + stepTime:
	...
	#track the sickle circle animation
		...
		if $Sprite2D.get_frame() == 1:
			$Sprite2D.set_offset(Vector2()) #reset the sprite offset, it was changed during the sickle charge
			$Sprite2D.set_scale(Vector2(1.075, 1.075))
		elif $Sprite2D.get_frame() == 2:
			$Sprite2D.set_scale(Vector2(1.15, 1.15))
		elif $Sprite2D.get_frame() == 3:
			$Sprite2D.set_scale(Vector2(1.075, 1.075))
			Zone.swing_sickle_circle(position)
		elif $Sprite2D.get_frame() == 4:
	...
	if lastAnimation == "pass out": #TODO requires code generalization for expansion of the game
		if $Sprite2D.get_frame() == 15 and not do_it_once: #player is done passing out
			do_it_once = true
...
func play_animation(x_multiplier, y_multiplier, frame_count, action):
	if $Sprite2D.get_frame() == frame_count-2:
		$Sprite2D.set_offset(Vector2($Sprite2D.get_frame() * x_multiplier, $Sprite2D.get_frame() * y_multiplier))
	elif $Sprite2D.get_frame() == frame_count-1:
		$Sprite2D.set_offset(Vector2($Sprite2D.get_frame()/2 * x_multiplier, $Sprite2D.get_frame()/2 * y_multiplier))
	elif $Sprite2D.get_frame() == frame_count:
		$Sprite2D.set_offset(Vector2()) #0, reset animation
		reset_animation()
	else:
		$Sprite2D.set_offset(Vector2($Sprite2D.get_frame() * 2 * x_multiplier, $Sprite2D.get_frame() * 2 * y_multiplier))
	
	#signal to change the tile
	if ($Sprite2D.get_frame() == 9 && action == "hammer"):
		Zone.smash_hammer(position, facingDirection)
	elif ($Sprite2D.get_frame() == 9 && action == "hoe"):
		Zone.swing_hoe(position, facingDirection)
	elif ($Sprite2D.get_frame() == 9 && action == "axe"):
		Zone.swing_axe(position, facingDirection)
	elif ($Sprite2D.get_frame() == 9 && action == "sickle"):
		Zone.swing_sickle(position, facingDirection)

func play_animation_water(x_multiplier, y_multiplier):
	if $Sprite2D.get_frame() == 22:
		$Sprite2D.set_offset(Vector2()) #0, reset animation
		reset_animation()
	elif $Sprite2D.get_frame() == 21:
		$Sprite2D.set_offset(Vector2(10 * x_multiplier, 10 * y_multiplier))
	elif ($Sprite2D.get_frame() == 17): #signal to change the tile
		Zone.water_square(position, facingDirection)
	elif $Sprite2D.get_frame() <= 10:
		$Sprite2D.set_offset(Vector2($Sprite2D.get_frame() * 2 * x_multiplier, $Sprite2D.get_frame() * 2 * y_multiplier))
...
func play_animation_pickup(x_multiplier, y_multiplier):
	...
	if crop_number == 17:
		if $Sprite2D.get_frame() == 0:
			$Sprite2D.set_offset(Vector2(10 * x_multiplier, 10 * y_multiplier))
		elif $Sprite2D.get_frame() == 1:
			$Sprite2D.set_offset(Vector2(20 * x_multiplier, 20 * y_multiplier))
		elif $Sprite2D.get_frame() == 2:
			$Sprite2D.set_offset(Vector2(10 * x_multiplier, 10 * y_multiplier))
...
func play_animation_store(x_multiplier, y_multiplier):
	...
	if crop_number == 17:
		...
		if facingDirection == "left" or facingDirection == "right":
			...
			elif $Sprite2D.get_frame() == 3: #reset Sprite2D
				Eggplant.hide()
